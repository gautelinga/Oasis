...
mesh_file = "git/Oasis/mesh/rough_duct.h5"
if path.isfile(mesh_file):
   info_red("Mesh: " + mesh_file)
else:
   exit("Couldn't find mesh.")
...
# Kept for later...
init_folder = "RoughDuctF6e-6_results/data/1/Timeseries/"
...
def build_wall_coords():
    if start_from_scratch or True:
        ...
        with h5py.File(h5fu_str, "r") as h5f:
            ...
            elems = np.array(h5f["Mesh/0/mesh/topology"])
...
def mesh(refine=1, **params):
    ...
    if start_from_scratch or True:
        ...
        h5fmesh = HDF5File(mesh.mpi_comm(), h5fp_str, "r")
        h5fmesh.read(mesh, "/Mesh/0/mesh", False)
    h5fmesh.close()
    return mesh
