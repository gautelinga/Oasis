...
from dolfin import MPI, Function, XDMFFile, HDF5File, info, \
    VectorFunctionSpace, MPI, FunctionAssigner
...
def create_initial_folders(folder, restart_folder, sys_comp, tstep, info_red,
    ...
    for ui in comps:
        tstepfiles[ui] = XDMFFile(MPI.comm_world, path.join(tstepfolder, ui+'_from_tstep_{}.xdmf'.format(tstep)))
...
def check_if_kill(folder):
    ...
    if collective > 0:
        if MPI.rank(MPI.comm_world) == 0:
            ...
            info('killoasis Found! Stopping simulations cleanly...')
        return True
    else:
        return False
